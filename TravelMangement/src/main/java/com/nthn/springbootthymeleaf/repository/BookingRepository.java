...
public interface BookingRepository extends JpaRepository<Booking, Integer>, JpaSpecificationExecutor<Booking> {
    ...
    List<Booking> findByBookingDateBetweenAndCustomerId(LocalDateTime fromDate, LocalDateTime toDate,
                                                        Integer customerId);
    ...
    @Query("select b from Booking b where b.customer.id=?1 and b.departureDate.date between ?2 and ?3 and b" +
            ".payment!=null")
    Page<Booking> findByCustomerIdPaid(Integer id, LocalDate fromDate, LocalDate toDate, Pageable pageable);
    
    @Query("select b from Booking b where b.customer.id=?1 and b.departureDate.date between ?2 and ?3 and b.payment=null")
    Page<Booking> findByCustomerIdNoPaid(Integer id, LocalDate fromDate, LocalDate toDate, Pageable pageable);
    
    @Query("select b from Booking b where b.customer.id=?1 and b.departureDate.date between ?2 and ?3")
    ...
    @Query("select count(b), sum(b.total) from Booking b where month(b.bookingDate)=?1 and year(b.bookingDate)=?2 and" +
            " b.payment!=null")
    List<Object[]> getRevenueMonthly(int month, int year);
    
    @Query(" select count(b), sum(b.total) from Booking b where year(b.bookingDate)=?1 and b.payment!=null")
