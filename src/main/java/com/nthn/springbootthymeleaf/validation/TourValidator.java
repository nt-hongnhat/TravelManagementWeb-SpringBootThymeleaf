	...
	public void validate(@NotNull Object target, @NotNull Errors errors) {
		...
		if (Objects.isNull(tour.getId())) {
			if (tour.getName().length() < 6) {
				errors.rejectValue(Field.Tour.NAME, "tour_info.name_LENGTH",
						"Tên tour " + "phải " + "có " + "ít " + "nhất 6 ký " + "tự.");
			}
			if (tour.getName().length() > 255) {
				errors.rejectValue(Field.Tour.NAME, "tour_info.name_TOO_LONG", "Tên tour quá dài.");
			}
			
			globalValidate.validateNotNull(tour.getUnitPrice(), Table.TOUR, Field.Tour.UNIT_PRICE,
					errors);
			...
			globalValidate.validateNotNull(tourService.getByName(tour.getName()), Table.TOUR,
					Field.Tour.NAME, errors);
		}
